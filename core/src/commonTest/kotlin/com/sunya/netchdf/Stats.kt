...
class Stat(val statName: String, val thing : String) {
    ...
    fun accum(nanosecs : Long, nthings : Int) {
        ...
        accum.addAndFetch(nanosecs)
        this.nthings.addAndFetch(nthings)
        count.addAndFetch(1)
    }
    ...
    fun accum() = this.accum.load()

    fun nthings() = this.nthings.load()

    fun count() = this.count.load()

    fun sum(other : Stat) {
        this.accum.addAndFetch(other.accum())
        this.count.addAndFetch(other.count())
        this.nthings.addAndFetch(other.nthings())
    }
}
