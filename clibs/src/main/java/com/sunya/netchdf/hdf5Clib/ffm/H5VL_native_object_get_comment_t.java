// Generated by jextract

package com.sunya.netchdf.hdf5Clib.ffm;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct H5VL_native_object_get_comment_t {
 *     size_t buf_size;
 *     void* buf;
 *     size_t* comment_len;
 * };
 * }
 */
public class H5VL_native_object_get_comment_t {

    public static MemoryLayout $LAYOUT() {
        return constants$433.const$0;
    }
    public static VarHandle buf_size$VH() {
        return constants$433.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * size_t buf_size;
     * }
     */
    public static long buf_size$get(MemorySegment seg) {
        return (long)constants$433.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * size_t buf_size;
     * }
     */
    public static void buf_size$set(MemorySegment seg, long x) {
        constants$433.const$1.set(seg, x);
    }
    public static long buf_size$get(MemorySegment seg, long index) {
        return (long)constants$433.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void buf_size$set(MemorySegment seg, long index, long x) {
        constants$433.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle buf$VH() {
        return constants$433.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * void* buf;
     * }
     */
    public static MemorySegment buf$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$433.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * void* buf;
     * }
     */
    public static void buf$set(MemorySegment seg, MemorySegment x) {
        constants$433.const$2.set(seg, x);
    }
    public static MemorySegment buf$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$433.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void buf$set(MemorySegment seg, long index, MemorySegment x) {
        constants$433.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle comment_len$VH() {
        return constants$433.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * size_t* comment_len;
     * }
     */
    public static MemorySegment comment_len$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$433.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * size_t* comment_len;
     * }
     */
    public static void comment_len$set(MemorySegment seg, MemorySegment x) {
        constants$433.const$3.set(seg, x);
    }
    public static MemorySegment comment_len$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$433.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void comment_len$set(MemorySegment seg, long index, MemorySegment x) {
        constants$433.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


