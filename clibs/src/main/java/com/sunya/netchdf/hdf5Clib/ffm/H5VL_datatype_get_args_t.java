// Generated by jextract

package com.sunya.netchdf.hdf5Clib.ffm;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct H5VL_datatype_get_args_t {
 *     H5VL_datatype_get_t op_type;
 *     union  args;
 * };
 * }
 */
public class H5VL_datatype_get_args_t {

    public static MemoryLayout $LAYOUT() {
        return constants$229.const$3;
    }
    public static VarHandle op_type$VH() {
        return constants$229.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * H5VL_datatype_get_t op_type;
     * }
     */
    public static int op_type$get(MemorySegment seg) {
        return (int)constants$229.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * H5VL_datatype_get_t op_type;
     * }
     */
    public static void op_type$set(MemorySegment seg, int x) {
        constants$229.const$4.set(seg, x);
    }
    public static int op_type$get(MemorySegment seg, long index) {
        return (int)constants$229.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void op_type$set(MemorySegment seg, long index, int x) {
        constants$229.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    /**
     * {@snippet :
     * union {
     *     struct  get_binary_size;
     *     struct  get_binary;
     *     struct  get_tcpl;
     * };
     * }
     */
    public static final class args {

        // Suppresses default constructor, ensuring non-instantiability.
        private args() {}
        public static MemoryLayout $LAYOUT() {
            return constants$229.const$5;
        }
        /**
         * {@snippet :
         * struct {
         *     size_t* size;
         * };
         * }
         */
        public static final class get_binary_size {

            // Suppresses default constructor, ensuring non-instantiability.
            private get_binary_size() {}
            public static MemoryLayout $LAYOUT() {
                return constants$228.const$4;
            }
            public static VarHandle size$VH() {
                return constants$230.const$0;
            }
            /**
             * Getter for field:
             * {@snippet :
             * size_t* size;
             * }
             */
            public static MemorySegment size$get(MemorySegment seg) {
                return (java.lang.foreign.MemorySegment)constants$230.const$0.get(seg);
            }
            /**
             * Setter for field:
             * {@snippet :
             * size_t* size;
             * }
             */
            public static void size$set(MemorySegment seg, MemorySegment x) {
                constants$230.const$0.set(seg, x);
            }
            public static MemorySegment size$get(MemorySegment seg, long index) {
                return (java.lang.foreign.MemorySegment)constants$230.const$0.get(seg.asSlice(index*sizeof()));
            }
            public static void size$set(MemorySegment seg, long index, MemorySegment x) {
                constants$230.const$0.set(seg.asSlice(index*sizeof()), x);
            }
            public static long sizeof() { return $LAYOUT().byteSize(); }
            public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
            public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
                return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
            }
            public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
        }

        public static MemorySegment get_binary_size$slice(MemorySegment seg) {
            return seg.asSlice(0, 8);
        }
        /**
         * {@snippet :
         * struct {
         *     void* buf;
         *     size_t buf_size;
         * };
         * }
         */
        public static final class get_binary {

            // Suppresses default constructor, ensuring non-instantiability.
            private get_binary() {}
            public static MemoryLayout $LAYOUT() {
                return constants$230.const$1;
            }
            public static VarHandle buf$VH() {
                return constants$230.const$2;
            }
            /**
             * Getter for field:
             * {@snippet :
             * void* buf;
             * }
             */
            public static MemorySegment buf$get(MemorySegment seg) {
                return (java.lang.foreign.MemorySegment)constants$230.const$2.get(seg);
            }
            /**
             * Setter for field:
             * {@snippet :
             * void* buf;
             * }
             */
            public static void buf$set(MemorySegment seg, MemorySegment x) {
                constants$230.const$2.set(seg, x);
            }
            public static MemorySegment buf$get(MemorySegment seg, long index) {
                return (java.lang.foreign.MemorySegment)constants$230.const$2.get(seg.asSlice(index*sizeof()));
            }
            public static void buf$set(MemorySegment seg, long index, MemorySegment x) {
                constants$230.const$2.set(seg.asSlice(index*sizeof()), x);
            }
            public static VarHandle buf_size$VH() {
                return constants$230.const$3;
            }
            /**
             * Getter for field:
             * {@snippet :
             * size_t buf_size;
             * }
             */
            public static long buf_size$get(MemorySegment seg) {
                return (long)constants$230.const$3.get(seg);
            }
            /**
             * Setter for field:
             * {@snippet :
             * size_t buf_size;
             * }
             */
            public static void buf_size$set(MemorySegment seg, long x) {
                constants$230.const$3.set(seg, x);
            }
            public static long buf_size$get(MemorySegment seg, long index) {
                return (long)constants$230.const$3.get(seg.asSlice(index*sizeof()));
            }
            public static void buf_size$set(MemorySegment seg, long index, long x) {
                constants$230.const$3.set(seg.asSlice(index*sizeof()), x);
            }
            public static long sizeof() { return $LAYOUT().byteSize(); }
            public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
            public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
                return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
            }
            public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
        }

        public static MemorySegment get_binary$slice(MemorySegment seg) {
            return seg.asSlice(0, 16);
        }
        /**
         * {@snippet :
         * struct {
         *     hid_t tcpl_id;
         * };
         * }
         */
        public static final class get_tcpl {

            // Suppresses default constructor, ensuring non-instantiability.
            private get_tcpl() {}
            public static MemoryLayout $LAYOUT() {
                return constants$230.const$4;
            }
            public static VarHandle tcpl_id$VH() {
                return constants$230.const$5;
            }
            /**
             * Getter for field:
             * {@snippet :
             * hid_t tcpl_id;
             * }
             */
            public static long tcpl_id$get(MemorySegment seg) {
                return (long)constants$230.const$5.get(seg);
            }
            /**
             * Setter for field:
             * {@snippet :
             * hid_t tcpl_id;
             * }
             */
            public static void tcpl_id$set(MemorySegment seg, long x) {
                constants$230.const$5.set(seg, x);
            }
            public static long tcpl_id$get(MemorySegment seg, long index) {
                return (long)constants$230.const$5.get(seg.asSlice(index*sizeof()));
            }
            public static void tcpl_id$set(MemorySegment seg, long index, long x) {
                constants$230.const$5.set(seg.asSlice(index*sizeof()), x);
            }
            public static long sizeof() { return $LAYOUT().byteSize(); }
            public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
            public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
                return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
            }
            public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
        }

        public static MemorySegment get_tcpl$slice(MemorySegment seg) {
            return seg.asSlice(0, 8);
        }
        public static long sizeof() { return $LAYOUT().byteSize(); }
        public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
        public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
            return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
        }
        public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
    }

    public static MemorySegment args$slice(MemorySegment seg) {
        return seg.asSlice(8, 16);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


