...
class CompareCdmWithClib(filename: String, showCdl: Boolean = false, showCompare: Boolean = false) {
    init {
        println("=============================================================")
            ...
            if (netchdf == null) {
                ...
                if (netchdf.type().contains("hdf4") || netchdf.type().contains("hdf-eos2")) {
                    Hdf4ClibFile(filename).use { ncfile ->
                        ...
                        compareNetchdfCdm(netchdf, ncfile, showCompare = showCompare)
                    }
...
fun compareNetchdfCdm(myfile: Netchdf, cfile: Netchdf, indent: Indent = Indent(2, startingLevel = 0), showCompare: Boolean = false) {
    compareGroup(myfile.rootGroup(), cfile.rootGroup(), indent, showCompare)
}

fun compareGroup(group: Group, cgroup: Group, indent: Indent, showCompare: Boolean) {
    if (showCompare) {
    ...
    group.attributes.forEach { att ->
        ...
        compareAttribute(att, catt, indent.incr(), showCompare)
    }
    ...
    group.groups.forEach { nested ->
        ...
        compareGroup(nested, cnested, indent.incr(), showCompare)
    }
}

fun compareAttribute(att: Attribute<*>, catt: Attribute<*>, indent: Indent, showCompare: Boolean) {
    if (showCompare) {
...
fun compareVariable(v: Variable<*>, cv: Variable<*>, indent: Indent, showCompare: Boolean) {
    if (showCompare) {
    ...
    compareDatatype(v.datatype, cv.datatype, indent.incr(), showCompare)
}

fun compareDatatype(datatype: Datatype<*>, cdatatype: Datatype<*>, indent: Indent, showCompare: Boolean) {
    if (showCompare) {
        println("${indent}datatype  ${datatype}")
        println("${indent}cdatatype ${cdatatype}")
    }
    assertEquals (datatype.cdlName, cdatatype.cdlName)
    compareTypedef(datatype.typedef, cdatatype.typedef, indent, showCompare)
}

fun compareTypedef(t: Typedef?, ct: Typedef?, indent: Indent, showCompare: Boolean) {
